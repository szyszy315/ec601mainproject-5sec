...
ALLOWED_EXTENSIONS = {'mp4', 'wmv', 'avi', 'jpg', 'png'}
...
def seg(path, filename):
    ...
    if not os.path.exists(newpath):
        os.makedirs(newpath)
    convert_videos_to_tfrecord(app.config['UPLOAD_FOLDER'], os.path.join(app.config['TFRECORD_FOLDER'],filename), 1, 2, filename)

    start_time = 1
    end_time = start_time + 5
    ffmpeg_extract_subclip(os.path.join(path, filename), start_time, end_time, targetname=os.path.join(app.config['DOWNLOAD_FOLDER'], filename))
